...
- (NSMutableArray *)loadDatas {
    ...
    for (NSDictionary *eachDic in JSONDic[@"data"][@"rows"]) {
        ...
        dyFrameModel *framemodel=[[dyFrameModel alloc] init];
        framemodel.model =[dyModel mj_objectWithKeyValues:eachDic];
...
- (void)setModel:(dyModel *)model
{
    _model = model;
    // 设置正文的frame
    CGFloat textHeight =  [model.message sizeWithFont:[UIFont systemFontOfSize:SectionHeaderContentFontSize] lineSpacing:SectionHeaderLineSpace maxSize:CGSizeMake(FrameW-SectionHeaderHorizontalSpace, MAXFLOAT)];
...
//    }
    ...
    self.HeadViewHeight = textHeight+20;

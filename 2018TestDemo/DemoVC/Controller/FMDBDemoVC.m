...
- (void)insert
{
    ...
    int ID =  arc4random_uniform(100);;
    //1.executeUpdate:不确定的参数用？来占位（后面参数必须是oc对象，；代表语句结束）
    BOOL result = [self.db executeUpdate:@"INSERT INTO t_student (ID,name, score, phone) VALUES (?,?,?,?)",@(ID),name,@(score),phone];
